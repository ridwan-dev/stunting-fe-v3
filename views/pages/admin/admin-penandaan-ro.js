import{apopoverTrigger,apiMasterIntervensi}from"../../../services/api.js";const AdminPenandaanRo={render:async()=>(mData.dataMasterIntervensi=void 0===mData.dataMasterIntervensi?await apiMasterIntervensi():mData.dataMasterIntervensi,`
    <div class="app-content-padding flex-grow-1 overflow-auto admin-cover" data-height="100%">
      ...
      <div class="modal fade" id="tileModal" tabindex="-1" aria-hidden="true">
    ...
    </div>`),after_render:async()=>{let a=document.querySelector("#formsub"),t=document.querySelector("#input-revisi-penandaan"),e=document.querySelector("#subdata"),i=document.querySelector("#pushdata");i.onclick=async function(){if(t.reportValidity()){let t=[],e=[],a=$("#input-revisi-penandaan").serializeArray();a.forEach(a=>{"ditandai"===a.name&&t.push(a.value),"disepakati"===a.name&&e.push(a.value)});try{let a=await fetch(config.api_url+"/renja/taggingpost",{method:"POST",body:JSON.stringify({tahun:$("#sel_tahun").val(),ditandai:t,disepakati:e}),headers:config.fetchHeaders});var i=await a.json();return console.log(i),i.status&&swal({title:"Data tersimpan",text:" ",icon:"success",buttons:{confirm:{text:"Tutup",value:!0,visible:!0,className:"btn btn-success",closeModal:!0}}}),i.data}catch(a){return!1}}},e.onclick=async function(){$("#beforeload").addClass("loading"),a.reportValidity()&&await async function(t){try{let a=await fetch(config.api_url+"/renja/listro-tagging",{method:"POST",body:JSON.stringify({tahun:t}),headers:config.fetchHeaders});var e=await a.json();return console.log(e.data),e.data}catch(a){return!1}}($("#sel_tahun").val()).then(function(a){$("#input-revisi-penandaan").removeClass("hide"),async function(a){const t=a.data;!function(a){$("#table-admin-penandaan-pagu").html(`
        ...
        `),$("#tableData").DataTable({scrollY:300,scrollX:!0,scrollCollapse:!0,paging:!1,fixedColumns:!0,responsive:!0,dom:"Bfrtip",buttons:["excel"]}),$("#ditandaiAll").click(function(a){var t=this.checked;$('input[check="ditandai"]').each(function(){this.checked=t})}),$("#disepakatiAll").click(function(a){var t=this.checked;$('input[check="disepakati"]').each(function(){this.checked=t})})}((()=>{let i=[];return t.sort((a,t)=>a.kementerian_kode>t.kementerian_kode?1:-1),console.log("xxx",t),t.forEach((a,t)=>{var e='<div class="badge '+c_main+'"><span class="badge-left '+c_kl+' p-1" title="K/L : '+a.kementerian_nama+'"  data-bs-toggle="popover" style="cursor:pointer">'+a.kementerian_kode+'</span><span class="'+c_prog+' p-1" title="Program : '+a.program_nama+'">'+a.program_kode+'</span><span class="'+c_keg+' p-1" title="Kegiatan : '+a.kegiatan_nama+'">'+a.kegiatan_kode+'</span><span class="'+c_kro+' p-1" title="KRO : '+a.output_nama+'">'+a.output_kode+'</span><span class="'+color_ro+' badge-right p-1" title="RO : '+a.suboutput_nama+'">'+a.suboutput_kode+"</span></div>";i.push(`<tr>
                <td style="">
                  ${t+1}.
                </td>
                <td style="">
                ...
                <td>
                  <div class="d-flex flex-row bd-highlight">
                    <div class="bd-highlight">${e}</div>
                    <div class="bd-highlight text-wrap">${a.suboutput_nama}</div>
                  </div>
                </td>
            </tr>`)}),i.join("")})())}(a)}),$("#beforeload").removeClass("loading")},apopoverTrigger()}};export default AdminPenandaanRo;